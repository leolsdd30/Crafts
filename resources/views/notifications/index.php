    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex items-center gap-2">
                ...
                <button onclick="markAllRead()"
                        class="inline-flex items-center px-3 py-2 text-sm font-medium text-indigo-600 bg-indigo-50 border border-indigo-200 rounded-lg hover:bg-indigo-100 transition duration-150">
                ...
                <button onclick="clearAll()"
                        class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-lg hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition duration-150">
        ...
        <div class="flex items-center gap-2 mb-6">
            <button id="filter-all-btn"
                    ...
                    class="px-3 py-1.5 text-xs font-semibold rounded-full transition duration-150 bg-indigo-600 text-white">
                All
            </button>
            <button id="filter-unread-btn"
                    onclick="setFilter('unread')"
                    class="inline-flex items-center px-3 py-1.5 text-xs font-semibold rounded-full transition duration-150 bg-white border border-gray-200 text-gray-500 hover:border-indigo-300 hover:text-indigo-600">
                Unread
                <span class="ml-1.5 inline-flex items-center justify-center w-4 h-4 text-xs font-bold rounded-full bg-indigo-100 text-indigo-700"><?= $unreadCount ?></span>
            </button>
        </div>
        <?php endif; ?>
        ...
        <div class="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-16 text-center">
            ...
            <p class="mt-1 text-sm text-gray-400">When something happens, you'll see it here.</p>
        </div>
        ...
        <?php else:
            ...
            $iconMap = [
                ...
                'booking_declined'  => ['bg' => 'bg-red-100',    'text' => 'text-red-500',    'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/>'],
                'booking_pending'   => ['bg' => 'bg-amber-100',  'text' => 'text-amber-600',  'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>'],
                ...
                'quote_new'         => ['bg' => 'bg-blue-100',   'text' => 'text-blue-600',   'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>'],
                'counter_offered'   => ['bg' => 'bg-orange-100', 'text' => 'text-orange-600', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/>'],
                'counter_rejected'  => ['bg' => 'bg-red-100',    'text' => 'text-red-500',    'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/>'],
                ...
                'message_new'       => ['bg' => 'bg-indigo-100', 'text' => 'text-indigo-600', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 12.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>'],
            ...
            $defaultIcon = ['bg' => 'bg-gray-100', 'text' => 'text-gray-500', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>'];
        ?>
        ...
        <div class="space-y-6" id="notifications-list">
            ...
            <div data-group="<?= strtolower(str_replace(' ', '-', $label)) ?>">
                ...
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <span class="group-unread-badge inline-flex items-center justify-center px-2 py-0.5 rounded-full text-xs font-bold bg-indigo-100 text-indigo-700">
                ...
                <div class="space-y-2">
                    ...
                    ?>
                    <div class="group relative bg-white rounded-xl shadow-sm border <?= $notif['is_read'] ? 'border-gray-100' : 'border-indigo-200 bg-indigo-50/20 ring-1 ring-indigo-100' ?> p-4 flex items-start gap-4 hover:shadow-md transition-all duration-200"
                         id="notif-<?= $notif['id'] ?>"
                        ...
                        <div class="flex-1 min-w-0 pr-8">
                            <div class="flex items-start justify-between gap-2">
                                <p class="text-sm font-semibold text-gray-900 leading-snug">
                                    ...
                                    <span class="inline-block w-2 h-2 bg-indigo-500 rounded-full ml-1 align-middle" id="unread-dot-<?= $notif['id'] ?>"></span>
                                    <?php endif; ?>
                                </p>
                            ...
                            <div class="flex items-center gap-4 mt-2">
                                ...
                                <button onclick="markOneRead(<?= $notif['id'] ?>)"
                                        ...
                                        class="inline-flex items-center text-xs font-medium text-gray-400 hover:text-indigo-600 transition-colors">
                        ...
                        <button onclick="dismissNotification(<?= $notif['id'] ?>)"
                                title="Dismiss"
                                class="absolute top-3 right-3 p-1 rounded-lg text-gray-500 hover:text-red-500 hover:bg-red-100 opacity-0 group-hover:opacity-100 transition-all duration-150">
        ...
        <div id="unread-empty" class="hidden bg-white rounded-2xl border-2 border-dashed border-gray-200 p-12 text-center mt-4">
            <div class="mx-auto h-14 w-14 bg-indigo-50 rounded-full flex items-center justify-center mb-3">
                <svg class="h-7 w-7 text-indigo-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
            </div>
            <h3 class="text-sm font-semibold text-gray-900">No unread notifications</h3>
...
function setFilter(filter) {
    ...
    if (filter === 'all') {
        allBtn.className    = 'px-3 py-1.5 text-xs font-semibold rounded-full transition duration-150 bg-indigo-600 text-white';
        unreadBtn.className = 'inline-flex items-center px-3 py-1.5 text-xs font-semibold rounded-full transition duration-150 bg-white border border-gray-200 text-gray-500 hover:border-indigo-300 hover:text-indigo-600';
    } else {
        unreadBtn.className = 'inline-flex items-center px-3 py-1.5 text-xs font-semibold rounded-full transition duration-150 bg-indigo-600 text-white';
        allBtn.className    = 'px-3 py-1.5 text-xs font-semibold rounded-full transition duration-150 bg-white border border-gray-200 text-gray-500 hover:border-indigo-300 hover:text-indigo-600';
    }
    applyFilter();
}
...
async function markOneRead(id) {
    ...
    if (el) {
        el.dataset.read = '1';
        el.classList.remove('border-indigo-200', 'bg-indigo-50/20', 'ring-1', 'ring-indigo-100');
