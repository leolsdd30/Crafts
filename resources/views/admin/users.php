...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-start justify-between">
                ...
                <a href="<?= APP_URL ?>/admin/verifications"
                   class="mt-6 inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 transition duration-150">
                    ...
                    <span class="ml-2 bg-yellow-400 text-yellow-900 text-xs font-bold px-1.5 py-0.5 rounded-full"><?= $stats['pending_verification'] ?></span>
                    <?php endif; ?>
                </a>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
                <p class="text-sm text-gray-500">
                    Showing <span class="font-semibold text-gray-700"><?= count($users) ?></span>
                    of <span class="font-semibold text-gray-700"><?= $totalUsers ?? count($users) ?></span>
                ...
                <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                    ...
                    <a href="?<?= $qs ?>&page=<?= $i ?>" class="relative inline-flex items-center px-3 py-1.5 text-sm font-semibold <?= $i === $page ? 'z-10 bg-indigo-600 text-white' : 'text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50' ?>">
                        <?= $i ?>
                    </a>
                    <?php endfor; ?>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php if (empty($users)): ?>
                        <tr><td colspan="7" class="px-6 py-12 text-center text-sm text-gray-500">No users found matching your criteria.</td></tr>
                        <?php endif; ?>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400 mt-0.5"><?= htmlspecialchars(preg_replace('/^\d{2}\s-\s/', '', $u['wilaya'])) ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php
                                    $roleColors = ['admin' => 'bg-red-100 text-red-800', 'craftsman' => 'bg-indigo-100 text-indigo-800', 'homeowner' => 'bg-green-100 text-green-800'];
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                ...
                                <form id="status-form-<?= $u['id'] ?>" method="POST" action="<?= APP_URL ?>/admin/users/toggle-status" class="inline">
                                    ...
                                    <?php if ($u['is_active']): ?>
                                    <button type="button" class="text-xs font-medium text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-colors"
                                    ...
                                    <?php else: ?>
                                    <button type="button" class="text-xs font-medium text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100 px-3 py-1.5 rounded-lg transition-colors"
        ...
        <!-- Bottom Pagination -->
        <?php if (isset($totalPages) && $totalPages > 1): ?>
        <div class="mt-6 flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6 rounded-xl shadow-sm">
            <div class="flex flex-1 justify-between sm:hidden">
                <a href="?<?= $qs ?>&page=<?= max(1, $page - 1) ?>" class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Previous</a>
                <a href="?<?= $qs ?>&page=<?= min($totalPages, $page + 1) ?>" class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Next</a>
            </div>
