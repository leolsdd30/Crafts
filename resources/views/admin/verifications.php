    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-start justify-between">
                ...
                <a href="<?= APP_URL ?>/admin/users"
                   class="mt-6 inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 transition duration-150">
        ...
        <div class="border-b border-gray-200 mb-6">
            <nav class="flex space-x-8">
                ...
                <a href="<?= APP_URL ?>/admin/verifications?filter=<?= $val ?>"
                   class="<?= $filter === $val ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' ?> whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors">
                    <?= $label ?>
                    <?php if ($filter === $val && $totalCraftsmen > 0): ?>
                    <span class="ml-2 bg-indigo-100 text-indigo-700 text-xs font-bold px-2 py-0.5 rounded-full"><?= $totalCraftsmen ?></span>
                    <?php endif; ?>
                </a>
                <?php endforeach; ?>
            </nav>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-6">
            <form method="GET" action="<?= APP_URL ?>/admin/verifications" class="flex flex-wrap items-end gap-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Wilaya</label>
                    <select name="wilaya" class="border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2 border bg-white">
                ...
                <div>
                    ...
                    <select name="category" class="border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2 border bg-white">
                ...
                <div>
                    ...
                    <select name="sort" class="border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2 border bg-white">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="<?= APP_URL ?>/admin/verifications?filter=<?= htmlspecialchars($filter) ?>" class="text-sm text-gray-500 hover:text-gray-700 transition-colors">Clear</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
            <p class="text-sm text-gray-500">
                Showing <span class="font-semibold text-gray-700"><?= count($craftsmen) ?></span>
                of <span class="font-semibold text-gray-700"><?= $totalCraftsmen ?></span>
                craftsman<?= $totalCraftsmen !== 1 ? 'en' : '' ?>
            </p>
            <?php if ($totalPages > 1): ?>
            <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                <?php if ($page > 1): ?>
                <a href="?<?= $qs ?>&page=<?= $page - 1 ?>" class="relative inline-flex items-center rounded-l-md px-2 py-1.5 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                ...
                <a href="?<?= $qs ?>&page=<?= $i ?>" class="relative inline-flex items-center px-3 py-1.5 text-sm font-semibold <?= $i === $page ? 'z-10 bg-indigo-600 text-white' : 'text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50' ?>"><?= $i ?></a>
                <?php endfor; ?>
                <?php if ($page < $totalPages): ?>
                <a href="?<?= $qs ?>&page=<?= $page + 1 ?>" class="relative inline-flex items-center rounded-r-md px-2 py-1.5 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
        ...
        <div class="bg-white rounded-xl border-2 border-dashed border-gray-200 p-12 text-center">
            ...
            <p class="mt-1 text-sm text-gray-500"><?= $filter === 'pending' ? 'No pending verifications. All caught up!' : 'No craftsmen match this filter.' ?></p>
        </div>
        <?php else: ?>
                ...
                <div class="p-6">
                    ...
                    <div class="flex items-center space-x-4 mb-4">
                        <img src="<?= get_profile_picture_url($c['profile_picture'] ?? 'default.png', $c['first_name'], $c['last_name']) ?>"
                             ...
                             class="h-14 w-14 rounded-full object-cover border-2 <?= $c['is_verified'] ? 'border-green-300' : 'border-yellow-200' ?>">
                        <div class="flex-1 min-w-0">
                            <!-- Name + verified badge inline -->
                            <div class="flex items-center gap-1.5">
                                <a href="<?= APP_URL ?>/profile/<?= htmlspecialchars($c['username']) ?>"
                                   class="text-base font-bold text-gray-900 hover:text-indigo-600 transition-colors truncate">
                            ...
                            <p class="text-sm text-gray-500 truncate"><?= htmlspecialchars($c['email']) ?></p>
                    ...
                    <div class="space-y-2 mb-4">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-medium text-gray-900"><?= htmlspecialchars($c['service_category']) ?></span>
                        </div>
                        <?php endif; ?>
                        <?php if (!empty($c['hourly_rate'])): ?>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-500">Hourly Rate</span>
                            <span class="font-medium text-gray-900"><?= number_format($c['hourly_rate'], 2) ?> DZD</span>
                        </div>
                        <?php endif; ?>
                        <div class="flex items-center justify-between text-sm">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <?php else: ?>
                            <span class="inline-flex items-center text-xs font-semibold text-yellow-700 bg-yellow-50 px-2 py-0.5 rounded-full">Pending</span>
                            <?php endif; ?>
                        </div>
                    </div>

                    <?php if (!empty($c['bio'])): ?>
                    <p class="text-xs text-gray-500 italic line-clamp-2 mb-4">"<?= htmlspecialchars(substr($c['bio'], 0, 120)) . (strlen($c['bio']) > 120 ? '...' : '') ?>"</p>
                    <?php endif; ?>
                    ...
                    <form id="verify-form-<?= $c['user_id'] ?>" method="POST" action="<?= APP_URL ?>/admin/verifications/toggle">
                        ...
                        <?php if ($c['is_verified']): ?>
                        <button type="button"
                                class="w-full inline-flex justify-center items-center px-4 py-2 border border-red-300 shadow-sm text-sm font-medium rounded-lg text-red-700 bg-white hover:bg-red-50 transition duration-150"
                        ...
                        <?php else: ?>
                        <button type="button"
                                class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-green-600 hover:bg-green-700 transition duration-150"
        ...
        <!-- Bottom Pagination -->
        <?php if ($totalPages > 1): ?>
        <div class="mt-8 flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6 rounded-xl shadow-sm">
            <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                ...
                <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                    <?php if ($page > 1): ?>
                    <a href="?<?= $qs ?>&page=<?= $page - 1 ?>" class="relative inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                    ...
                    <?php endif; ?>
                    <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
                    <a href="?<?= $qs ?>&page=<?= $i ?>" class="relative inline-flex items-center px-4 py-2 text-sm font-semibold <?= $i === $page ? 'z-10 bg-indigo-600 text-white' : 'text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50' ?>"><?= $i ?></a>
                    <?php endfor; ?>
                    <?php if ($page < $totalPages): ?>
                    <a href="?<?= $qs ?>&page=<?= $page + 1 ?>" class="relative inline-flex items-center rounded-r-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <div class="flex items-center mb-4">
                <div id="modal-icon-accept" class="hidden h-10 w-10 rounded-full bg-green-100 flex items-center justify-center mr-3">
                ...
                <div id="modal-icon-decline" class="hidden h-10 w-10 rounded-full bg-red-100 flex items-center justify-center mr-3">
