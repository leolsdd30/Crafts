            ...
            <form action="<?= APP_URL ?>/search" method="GET" class="bg-white p-6 shadow-sm rounded-lg space-y-4">
                ...
                <div class="flex flex-col md:flex-row gap-3">
                    ...
                    <div class="flex gap-2">
                        <button type="button" onclick="document.getElementById('filter-section').classList.toggle('hidden'); this.querySelector('svg').classList.toggle('-scale-y-100');" class="inline-flex justify-center items-center px-4 py-2.5 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200">
                ...
                <div id="filter-section" class="<?= (!empty($filters['category']) || !empty($filters['wilaya']) || !empty($filters['sort'])) ? '' : 'hidden' ?> transition-all duration-300 ease-in-out">
