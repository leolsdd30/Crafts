...
class Message extends Model
{
     ...
     * If a booking exists between them, auto-accept.
     * Otherwise, set as 'pending' (message request).
     */
    public function findOrCreateConversation($userId, $otherUserId)
    {
        ...
        // Check if a booking exists between them → auto-accept
    ...
    {
        // Only the participant (recipient) can accept
     ...
     * Call this from BookingController when a booking is accepted/created.
     */
    ...
    /**
     * Get accepted conversations for a user (main inbox).
     */
    public function getAcceptedConversations($userId)
    {
        $sql = "SELECT 
                ...
                LEFT JOIN users u ON u.id = (CASE WHEN c.initiator_id = :uid3 THEN c.participant_id ELSE c.initiator_id END)
